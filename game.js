...
let camera={x:0,y:0},keys=new Set(),audio=null,mapOK=false,sanderOK=false,nemesisOK=false,mapCanvas=null,mapData=null;
...
function resize(){W=innerWidth;H=innerHeight;dpr=Math.min(devicePixelRatio||1,2);canvas.width=W*dpr;canvas.height=H*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);drawMini()}
...
function buildMapMask(){
  ...
  try{mapData=c.getImageData(0,0,MAP.w,MAP.h).data}catch{mapData=null}
}
...
$("startBtn").onclick=()=>{show("intro",false);show("difficulty",true);beep(440,.08)};
...
function start(){
 ...
 elapsed=0;energy=[];parts=[];texts=[];seed();paused=false;game=true;show("difficulty",false);show("hud",true);show("touch",true);
...
function update(dt){
 ...
 camera.x=clamp(sand.x-W/2,0,Math.max(0,MAP.w-W));camera.y=clamp(sand.y-H/2,0,Math.max(0,MAP.h-H));
...
function character(o,player){
 ...
 if(player&&sanderOK||!player&&nemesisOK){
  const im=player?imgs.sander:imgs.nemesis,fw=im.width/4,fh=im.height/4,row=Math.abs(Math.round(o.f.y))?2:0,col=Math.floor(elapsed*8)%4;
  ctx.drawImage(im,col*fw,row*fh,fw,fh,-o.r*1.15,-o.r*1.15,o.r*2.3,o.r*2.3)
 }else if(player){
  ctx.fillStyle="#167fe4";ctx.beginPath();ctx.arc(0,-2,o.r,0,Math.PI*2);ctx.fill();ctx.fillStyle="#111b2b";ctx.fillRect(-o.r*.72,7,o.r*1.44,o.r*.8);ctx.fillStyle="#111";ctx.fillRect(-o.r*.58,-o.r*.85,o.r*1.16,o.r*.43);ctx.fillStyle="#55caff";ctx.fillRect(-5,-7,10,4)
 }else{
  ctx.fillStyle="#f3f3ec";ctx.beginPath();ctx.ellipse(0,0,o.r*.9,o.r*1.1,0,0,Math.PI*2);ctx.fill();
  ctx.fillStyle="#e8edf0";ctx.beginPath();ctx.moveTo(-8,-20);ctx.lineTo(-17,-48);ctx.lineTo(-2,-28);ctx.lineTo(8,-49);ctx.lineTo(12,-19);ctx.fill();
  ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(-9,-3,5,0,Math.PI*2);ctx.fill();ctx.fillStyle="#111";ctx.beginPath();ctx.arc(-9,-3,2.4,0,Math.PI*2);ctx.fill();
  ctx.fillStyle="#d3b36a";ctx.beginPath();ctx.moveTo(8,-2);ctx.lineTo(25,4);ctx.lineTo(8,9);ctx.fill();
 }
...
function draw(){
 ctx.clearRect(0,0,W,H);ctx.save();ctx.translate(-camera.x,-camera.y);world();drawEnergy();character(sand,true);character(nem,false);
 for(const p of parts){ctx.globalAlpha=Math.max(0,p.life/.9);ctx.fillStyle=p.color;ctx.beginPath();ctx.arc(p.x,p.y,3,0,Math.PI*2);ctx.fill()}ctx.globalAlpha=1;
 for(const t of texts){ctx.globalAlpha=t.life;ctx.font="900 14px system-ui";ctx.textAlign="center";ctx.fillStyle="#fff";ctx.strokeStyle="#000b";ctx.lineWidth=4;ctx.strokeText(t.s,t.x,t.y);ctx.fillText(t.s,t.x,t.y)}ctx.restore();drawMini()
}
function drawMini(){
 const r=mini.getBoundingClientRect(),mw=Math.max(240,Math.floor(r.width||400)),mh=Math.floor(mw*MAP.h/MAP.w);mini.width=mw*dpr;mini.height=mh*dpr;mctx.setTransform(dpr,0,0,dpr,0,0);
 if(mapOK)mctx.drawImage(imgs.map,0,0,mw,mh);else{mctx.fillStyle="#245f31";mctx.fillRect(0,0,mw,mh);mctx.fillStyle="#e4c278";for(const y of [.17,.41,.62,.84])mctx.fillRect(0,mh*y,mw,mh*.07);for(const x of [.1,.28,.45,.66,.84])mctx.fillRect(mw*x,0,mw*.05,mh)}
 for(const e of energy)if(e.active){mctx.fillStyle="#ffd52b";mctx.beginPath();mctx.arc(e.x/MAP.w*mw,e.y/MAP.h*mh,2,0,Math.PI*2);mctx.fill()}
 mctx.strokeStyle="#fff9";mctx.lineWidth=1;mctx.strokeRect(camera.x/MAP.w*mw,camera.y/MAP.h*mh,Math.min(W,MAP.w)/MAP.w*mw,Math.min(H,MAP.h)/MAP.h*mh);
